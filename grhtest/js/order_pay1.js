$(document).ready(function(){
	//支付方式1.表示月卡2.表示微信3.表示银行卡4.表示支付宝
	var pay_style='1';
	...
	if (sessionStorage.orderCode) {
		...
		source="preOrderCode"+preOrderCode;
		sign=md5(source+"key"+common.secretKey()).toUpperCase();
		getOrderInfo(preOrderCode,sign,source);
	}
    ...
    function orderPayShow(data){
    	...
    	if (isCanPay) {
			...
			$('.pay_style_msg').show();
			$('.go_pay').css("background","#b2b2b2").show();
		}
    ...
    function headerShow(data){
    	...
    	$(".orderList_details ul li").eq(0).html("预购商品:<span>"+data.data.goodsInfo.goodsName+"</span>");
    	$(".orderList_details ul li").eq(1).html("订单号:<span>"+data.data.orderCode+"</span>");
    	$(".orderList_details ul li").eq(2).html("预购金额:<span class='font_color'>￥"+data.data.frontMoney+"</span>");
    };
    //点击选择支付方式
    $(".pay_style_box .pay_style_item").on("click",function(){
    	$(".pay_style_item").removeClass("bg_select");
    	$(this).addClass("bg_select")
    	if ($(this).find("p").html()=="月卡支付") {
    		pay_style="1";
    		if (isCanPay) {
				$('.pay_style_msg1').show();
				$('.go_pay').css("background","#fe7831").show();
    ...
    $(".go_pay").on("click",function(){
    	if ($(this).css("background-color")=="rgb(254, 120, 49)") {
    		if (pay_style=="1") {
	    		////console.log("月卡")
	    		smsCode=$("#verify_code1").val();
	    		if ($("#verify_code1").val()=="") {
	    			common.prompt("请输入验证码");
	    		} else if($("#verify_code1").val()==verify_code){
	    			////console.log("调动月卡支付");
	    			true_pay_month(preOrderCode,smsCode,mobile,sign,source);
	    		}else{
	    			common.prompt("验证码输入有误！");
	    		}
	    	} else if (pay_style=="2") {
	    		////console.log("调用微信支付");
	    		ture_pay_weixin(preOrderCode,openId,sign,source)
	    	} else if (pay_style=="3") {
	    		if ($("#pay_style_card").val()=="") {
	    			common.prompt("请输入银行卡号")
	    		}else{
	    			bankCard=$("#pay_style_card").val();
	    			ture_pay_yinghangka(preOrderCode,bankCard,sign,source);
	    		}
	    	} else if (pay_style=="4") {
	    		true_pay_zfb(preOrderCode,sign,source);
	    	}
    	}
    });
    /*月卡支付*/
    function true_pay_month(orderCode,smsCode,mobile,sign,source){
    	////console.log("orderCode"+orderCode+",sms"+smsCode+",mobile"+mobile);
		$.ajax({
			url:common.http,
	        type:"post",
	        dataType:"jsonp",
	        data:{
				method:'pre_order_month_pay',
				orderCode:orderCode,
				smsCode:smsCode,
				mobile:mobile,
				tokenId:common.tokenId(),
	...
	//支付宝支付
	function true_pay_zfb(preOrderCode,sign,source){
		//console.log(preOrderCode)
		$.ajax({
			url:common.http,
			type:"post",
	        dataType:"jsonp",
