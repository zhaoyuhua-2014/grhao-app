	...
	require(['common','goshopCar','LArea','mobileUi'],function(common,cart,area){
		...
		pub.plain.apiHandle = {
			...
			shop_cart_submit : {
				...
				couponList : function( couponInfolist ){
					...
					nodeBox.removeClass('hidden').find('.set_coupon_box1_bottom').html( html );
		...
		pub.apiHandle = {
			...
			storeInfo : {
				...
				apiData : function( d ){
					...
					//watm机器处理
					pub.firmIdType == 5 && node.find(".set_job_time").remove();
				}
			},
			// 默认地址
		...
		pub.eventHandle = {
			...
			init : function(){
				...
				//watm新增效果
				if (pub.firmIdType == '5') {
					$(".set_charge_contact_right").find(".take_own").remove().end().find(".take_others").html("自助售货机").addClass("actived");
					pub.pickUpMethod = 1;
					$(".set_order_remark").addClass("hidden");
				}
