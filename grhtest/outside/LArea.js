...
define(['jquery','LAreaData'],function($){
	var LAreaData = window.LAreaData;
    $.dtd = $.Deferred();
    //对象
    ...
    $.dtd.done(function(LAreaData,num){
        ...
        area.init( area.paramInit );

        area.value = [ 0, 0, 0 ];//控制初始位置，注意：该方法并不会影响到input的value

        $(document).on('click','.gearArea',function(){
            $(this).remove();
        });
        $(document).on('click','.area_ctrl',function( e ){
            e.stopPropagation();
        });
    });
