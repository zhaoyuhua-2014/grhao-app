	...
	pub.eventHandle.init = function(){
		...
		if (!pub.logined) {
			$('.main_top_login a').on("click",function(){
				...
				if(common.jumpMake.getItem() == 5){*/
				common.jumpLinkPlainApp('登录','html/login.html?type='+5);
				/*}else{
					alert(common.jumpMake.getItem())
				}*/
			})
		}
	...
	pub.userInfoRepaired.eventHandle = {
		init : function(){
			...
			$('.header_left').click(function(){
				...
				!isHide && pub.userInfoRepaired.switchInput('修改信息','.zs_phone_box','.zs_address_box');
			});
	...
	pub.coupon.apiHandle = {
		...
		couponInfo_manager : {
			...
			apiData : function( d ){
			    ...
			    if( data.list == 0 ){ $('.lodemore').html('没有更多数据了').show();return;  }
