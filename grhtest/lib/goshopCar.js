	...
	var goshopCar={
		...
		car_goods : function(){
			...
			/*for (var i = 0; i < arr.length; i++) {
				html += '<li class="list" data="' + arr[i].goodsId + '" packageNum="' + arr[i].packageNum + '" maxCount="' + arr[i].maxCount + '" price="' + arr[i].price + '">';
				html += 	'<ul class="sub-ul-box">';
				html += 		'<li class="sub-list select-node"><span class="select ' + ['unselect-dot','select-dot'][ arr[i].status ] + '"></span></li>';
				...
				html +=					'<p class="oprate"><span class="minus_num actived">－</span><input class="show_num" value="' + arr[i].count + '" disabled="disabled" readonly="true" dataID="'+arr[i].goodsId+'"><span class="add_num">+</span></p>';
				html +=				'</div>';
				html +=			'</li>'
